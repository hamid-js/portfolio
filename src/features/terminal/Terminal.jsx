...
const Terminal = () => {
  ...
  const handleEnter = () => {
    ...
    const commands = {
      help: ( <div className="flex flex-col items-center justify-around">
  ...
  const text = `a passionate web developer and
    ...
    Also, I'm open to remote and in-person positions. 
    You can reach me at `;

  return (
    <div className="xl:p-auto lg:w-6/6 h-full w-full overflow-hidden rounded-md bg-[--color-black] text-[--primary-color-300] shadow-md lg:px-32 lg:py-20">
      <div className="w-4/4 m-auto pb-5 pt-20 xl:text-2xl">
        Hi,
        ...
        <div>
          <Typewriter text={text} maxWidth={600} wordDelay={20} />
        </div>
        <div className="my-10 flex gap-4 text-[var(--secondary-color-600)]">
          <a href="">Email</a>
          <a href="">Telegram</a>
      ...
      <div>
        {output.map((entry, index) => (
          <div className="mt-5" key={index}>
            <span className="my-9 text-green-400 ">{`~/hamid-mohamadi>`} </span>
            {entry.command}
            <br />
        ...
        <div>
          <span className="text-green-400">{'~/hamid-mohamadi>'}</span>
          <input
            ref={inputRef}
            type="text"
            value={input}
            ...
            className="ml-2 mt-7 border-none bg-transparent text-white focus:outline-none"
          />
        </div>
      </div>
    </div>
